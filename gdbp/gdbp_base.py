...
#           opt:optax.chain(optax.adamw(learning_rate=1e-4, weight_decay=1e-5)
# )
#          ):
...
def train(model: Model,
          ...
          batch_size: int = 500,
          n_iter=None,
          optimizer=optax.chain(optax.adamw(learning_rate=1e-4, weight_decay=1e-5))):
    ...
    for i, (y, x) in tqdm(enumerate(batch_gen), total=n_iter, desc='training', leave=False):
        ...
        loss, new_opt_state, new_module_state, new_params = update_step(
            model.module, optimizer, opt_state, module_state, y, x, aux, const, sparams)
