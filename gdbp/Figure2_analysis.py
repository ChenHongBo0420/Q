...
# ============================
# 7) Analysis-only helpers: step-wise chain Jacobian diagnostics
# ============================
#
# These functions are ADDITIVE: they do not change the default training / test
# behavior above. They are only used when you explicitly call the analysis API.
#
# Main public entry points:
#   enable_fdbp_analysis_mode()
#   build_fdbp_analysis_module(...)
#   analysis_stepwise_chain_probe(...)
#
# The goal is to expose per-step FDBP states and suffix replay so that you can
# analyze step-wise residual-aligned chain amplification inside FDBP.

import importlib as _importlib
import pandas as _pd
import commplax.module.layer as _analysis_layer_mod

_ANALYSIS_ORIG_FDBP = core.fdbp
_ANALYSIS_PATCHED = False


def _analysis_l2_norm(x):
    x = jnp.asarray(x)
    return jnp.sqrt(jnp.real(jnp.vdot(x.reshape(-1), x.reshape(-1))) + 1e-12)


def _analysis_normalize_vec(x):
    return x / _analysis_l2_norm(x)


def _analysis_truth_slice_by_t(ds_probe, t_obj):
    x = np.asarray(ds_probe.x)
    s = max(0, int(t_obj.start))
    e = min(x.shape[0], int(t_obj.stop))
    return x[s:e]


def _analysis_find_fdbp_param_subtree(tree):
    if isinstance(tree, (dict, flax.core.FrozenDict)):
        keys = set(tree.keys())
        if any(str(k).startswith('DConv_') for k in keys) and any(str(k).startswith('NConv_') for k in keys):
            return tree
        for v in tree.values():
            found = _analysis_find_fdbp_param_subtree(v)
            if found is not None:
                return found
    return None


def _analysis_make_fdbp_vars_from_fullparams(full_params, template_vars):
    fdbp_params = _analysis_find_fdbp_param_subtree(full_params)
    if fdbp_params is None:
        raise RuntimeError('Could not find FDBP params subtree containing DConv_i / NConv_i')
    vars_out = {}
    for k, v in template_vars.items():
        if k == 'params':
            continue
        vars_out[k] = v
    vars_out['params'] = fdbp_params
    return vars_out


def _analysis_fdbp_stepdiag(
    scope,
    signal,
    steps=3,
    dtaps=261,
    ...
    """
    Drop-in replacement for core.fdbp with two extras:
      - diag_record=True : store per-step x_i, t_i in collection 'diag'
      - start_step / inject_signal:
            replay ONLY the suffix starting at start_step,
            with x,t injected at that boundary.
    """
    x, t = signal
    dconv = core.vmap(core.wpartial(core.conv1d, taps=dtaps, kernel_init=d_init))

    if diag_record:
        scope.variable('diag', 'x_input', lambda v: v, x).value = x
        scope.variable('diag', 't_input', lambda v: v, t).value = t
    ...
    for i in range(steps):
        ...
        if i == start_step and start_step > 0:
            if inject_signal is None:
                raise ValueError('inject_signal must be provided when start_step > 0')
            x, t = inject_signal

        x, td = scope.child(dconv, name='DConv_%d' % i)(core.Signal(x, t))
        c, t = scope.child(core.mimoconv1d, name='NConv_%d' % i)(
            core.Signal(jnp.abs(x) ** 2, td),
            taps=ntaps,
            kernel_init=n_init
        )
        ...
        if diag_record:
            scope.variable('diag', f'x_{i}', lambda v: v, x).value = x
            scope.variable('diag', f't_{i}', lambda v: v, t).value = t

    return core.Signal(x, t)


def enable_fdbp_analysis_mode():
    """
    Enable an analysis-only FDBP mode that exposes per-step state and suffix replay.

    Important:
      layer.FDBP captures core.fdbp at import time, so after patching core.fdbp
      we must reload commplax.module.layer and rebind the global `layer` symbol
      in this Figure2 module.
    """
    global layer, _ANALYSIS_PATCHED
    if _ANALYSIS_PATCHED:
        return layer

    core.fdbp = _analysis_fdbp_stepdiag
    _importlib.reload(_analysis_layer_mod)
    layer = _analysis_layer_mod
    _ANALYSIS_PATCHED = True
    return layer


def disable_fdbp_analysis_mode():
    """Restore original core.fdbp and reload layer."""
    global layer, _ANALYSIS_PATCHED
    core.fdbp = _ANALYSIS_ORIG_FDBP
    _importlib.reload(_analysis_layer_mod)
    layer = _analysis_layer_mod
    _ANALYSIS_PATCHED = False
    return layer
...
def build_fdbp_analysis_module(data: gdat.Input,
                               steps: int = 3,
                               dtaps: int = 261,
                               ntaps: int = 41,
                               xi: float = 1.1,
                               init_fn: Optional[tuple] = None):
    """
    Build an FDBP-only analysis module plus template variables.

    This is separate from the full Figure2 serial model. It is only used for
    step-wise internal diagnostics of the FDBP block.
    """
    enable_fdbp_analysis_mode()
    if init_fn is None:
        init_fn = fdbp_init(data.a, xi=xi, steps=steps)
    d_init, n_init = init_fn
    mod = layer.FDBP(
        steps=steps,
        dtaps=dtaps,
    ...
    y0 = jnp.asarray(data.y)
    z0, vars0 = mod.init(random.PRNGKey(0), core.Signal(y0))
    return mod, vars0


def analysis_stepwise_chain_probe(ds_probe: gdat.Input,
                                  full_params: Dict,
                                  steps: int = 3,
                                  dtaps: int = 261,
                                  ntaps: int = 41,
                                  xi: float = 1.1,
                                  fdbp_mod=None,
                                  fdbp_template_vars=None,
                                  return_per_step: bool = True):
    """
    Compute step-wise residual-aligned chain amplification INSIDE FDBP.

    For each step k:
      - use actual local residual e_k at the post-step state x_k
      - build suffix map from step k+1 to final FDBP output
      - propagate u_k = normalized(e_k)
      - measure harmful projection onto final residual r_K

    Returns:
      summary: dict with keys
        chain_proj_max
        chain_gain_at_maxproj
        chain_align_at_maxproj
        chain_step_at_maxproj
        final_res_norm
      steps_df (optional): DataFrame of all steps
    """
    enable_fdbp_analysis_mode()

    if fdbp_mod is None or fdbp_template_vars is None:
        fdbp_mod, fdbp_template_vars = build_fdbp_analysis_module(
            ds_probe, steps=steps, dtaps=dtaps, ntaps=ntaps, xi=xi
        )

    fdbp_vars = _analysis_make_fdbp_vars_from_fullparams(full_params, fdbp_template_vars)
    y0 = jnp.asarray(ds_probe.y)

    def _run_fdbp_with_diag(y_in):
        sig_out, mut = fdbp_mod.apply(
            fdbp_vars,
            core.Signal(jnp.asarray(y_in)),
            diag_record=True,
            mutable=['diag']
        )
        return sig_out, mut['diag']

    sig_final, diag = _run_fdbp_with_diag(y0)
    x_final = jnp.asarray(sig_final.val)
    t_final = sig_final.t

    ref_final = jnp.asarray(_analysis_truth_slice_by_t(ds_probe, t_final))
    TF = int(min(x_final.shape[0], ref_final.shape[0]))
    if TF <= 4:
        summary = dict(
            chain_proj_max=np.nan,
            chain_gain_at_maxproj=np.nan,
            chain_align_at_maxproj=np.nan,
            chain_step_at_maxproj=np.nan,
            final_res_norm=np.nan,
        )
        return (summary, _pd.DataFrame()) if return_per_step else summary

    x_final = x_final[:TF]
    ref_final = ref_final[:TF]
    r_final = (x_final - ref_final).reshape(-1)
    r_final_norm = _analysis_l2_norm(r_final)

    if float(r_final_norm) < 1e-10:
        summary = dict(
            chain_proj_max=np.nan,
            chain_gain_at_maxproj=np.nan,
            chain_align_at_maxproj=np.nan,
            chain_step_at_maxproj=np.nan,
            final_res_norm=float(r_final_norm),
        )
        return (summary, _pd.DataFrame()) if return_per_step else summary
    ...
    for k in range(steps):
        x_key = f'x_{k}'
        t_key = f't_{k}'
        if x_key not in diag or t_key not in diag:
            continue

        xk = jnp.asarray(diag[x_key])
        tk = diag[t_key]

        refk = jnp.asarray(_analysis_truth_slice_by_t(ds_probe, tk))
        Tk = int(min(xk.shape[0], refk.shape[0]))
        if Tk <= 4:
            continue
        ...
        refk = refk[:Tk]
        ek = xk - refk
        ek_norm = _analysis_l2_norm(ek)
        if float(ek_norm) < 1e-10:
            continue

        uk = _analysis_normalize_vec(ek)
        suffix_start = k + 1

        def G_suffix(x_in):
            x_in = jnp.asarray(x_in)
            if suffix_start >= steps:
                out = x_in
            else:
                sig_out = fdbp_mod.apply(
                    fdbp_vars,
                    core.Signal(y0),
                    start_step=suffix_start,
                    inject_signal=core.Signal(x_in, tk),
                )
                out = sig_out.val
        ...
        _, Ju = jax.jvp(G_suffix, (xk,), (uk,))
        Ju = Ju.reshape(-1)

        Ju_norm = _analysis_l2_norm(Ju)
        proj = jnp.real(jnp.vdot(Ju, r_final)) / (_analysis_l2_norm(uk) * r_final_norm + 1e-12)
        align = jnp.real(jnp.vdot(Ju, r_final)) / (Ju_norm * r_final_norm + 1e-12)
        gain = Ju_norm / (_analysis_l2_norm(uk) + 1e-12)

        step_rows.append(dict(
            step=k,
            chain_proj=float(proj),
            chain_align=float(align),
            chain_gain=float(gain),
            local_res_norm=float(ek_norm),
        ))
    ...
    dstep = _pd.DataFrame(step_rows)
    imax = int(np.nanargmax(dstep['chain_proj'].values))
    best = dstep.iloc[imax]
    summary = dict(
        chain_proj_max=float(best['chain_proj']),
        chain_gain_at_maxproj=float(best['chain_gain']),
        chain_align_at_maxproj=float(best['chain_align']),
        chain_step_at_maxproj=int(best['step']),
        final_res_norm=float(r_final_norm),
    )
    return (summary, dstep) if return_per_step else summary
